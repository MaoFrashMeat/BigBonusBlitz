...
public static class ShopUiProbe
{
    ...
    public static void Run()
    {
        ...
        string output = Environment.GetEnvironmentVariable("SHOP_UI_OUTPUT");
        ...
        typeof(MapScreen).GetMethod("OpenShop", flags).Invoke(host, null);
        ...
        foreach (var size in new[] { new Vector2Int(2556,1179) })
        {
            ...
            File.WriteAllBytes(Path.Combine(output,"shop-"+size.x+".png"),image.EncodeToPNG());
            foreach (var tabName in new[] { "TabSupply", "TabStats" })
            {
                Button tb = null; foreach (var b in canvas.GetComponentsInChildren<Button>(true)) if (b.name == tabName) { tb = b; break; }
                if (tb == null) continue;
                tb.onClick.Invoke();
                Canvas.ForceUpdateCanvases();
                foreach (var g in canvas.GetComponentsInChildren<Graphic>()) { g.SetAllDirty(); g.Rebuild(CanvasUpdate.PreRender); g.canvasRenderer.cull = false; }
                ...
                image.ReadPixels(new Rect(0,0,size.x,size.y),0,0); image.Apply();
                File.WriteAllBytes(Path.Combine(output,"shop-"+tabName+".png"),image.EncodeToPNG());
            }
        ...
        foreach (var button in canvas.GetComponentsInChildren<Button>())
            if (((RectTransform)button.transform).rect.height < 44) { }
