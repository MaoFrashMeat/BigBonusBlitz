...
namespace BBB.Runtime
{
    ...
    public sealed class TitleScreen : MonoBehaviour
    {
        ...
        private void BuildSettings(Transform stage)
        {
            ...
            {
                AzureUiControls.Label(page, "ResetLabel", 0, 132, 426, 36, "セーブデータを消して最初から", 21, true);
                AtelierUi.Text(page, "ResetDesc", 0, 76, 426, 64, "ソウル・装備・進み具合・実績がすべて消え、\n序章から始まります。音量などの設定は残ります。", 14, AtelierUi.Sub);
                Text state = AtelierUi.Text(page, "ResetState", 0, 12, 426, 38, "", 13, UiSkin.Hex("#ffb2b4"), true, TextAnchor.MiddleCenter);
                Button b = null;
                b = AtelierUi.Button(page, "ResetSave", 0, -45, 320, "消して最初から始める", () =>
                {
                    if (_confirmUntil > 0f && Time.time <= _confirmUntil) { _audio.UiPop(); Begin(true); return; }
