...
namespace BBB.Runtime
{
    ...
    public static class ShopScreen
    {
        ...
        public static GameObject Build(Transform stage, SlotMachine m, AudioManager audio, System.Action onSoulsChanged, System.Action onClose)
        {
            ...
            var overlay = UiFactory.Panel(stage, "ShopOverlay", Vector2.zero, new Vector2(4000, 4000), new Color(0, 0, 0, 0.68f));
            ...
            var title = UiFactory.Label(card, "Title", new Vector2(-W * 0.5f + 130, H * 0.5f - 28), new Vector2(240, 28), "街のショップ", 20, TextAnchor.MiddleLeft, UiSkin.Text);
            title.fontStyle = FontStyle.Bold;
            var soul = UiSkin.Number(card, "Soul", new Vector2(W * 0.5f - 120, H * 0.5f - 28), new Vector2(200, 28), "", 20, UiSkin.Hex("#8f6bff"));
            UiSkin.Img(card, "Line", new Vector2(0, H * 0.5f - 48), new Vector2(W - 36, 1), null, new Color(1, 1, 1, 0.09f));
            UiSkin.IconButton(card, "Close", new Vector2(W * 0.5f - 26, H * 0.5f - 26), 30, "×", () => onClose?.Invoke(), UiSkin.Btn, 17);
            ...
            Button tabGear = null, tabSupply = null;
            void SetTab(bool supply)
            {
                gridRoot.gameObject.SetActive(!supply);
                supplyRoot.gameObject.SetActive(supply);
                if (tabGear != null) UiSkin.SetButtonColor(tabGear, supply ? UiSkin.Btn : UiSkin.Blue, supply ? UiSkin.Text : UiSkin.Bg);
                if (tabSupply != null) UiSkin.SetButtonColor(tabSupply, supply ? UiSkin.Green : UiSkin.Btn, supply ? UiSkin.Bg : UiSkin.Text);
            }
            if (hasSupply)
            {
                tabGear = UiSkin.Button(card, "TabGear", new Vector2(-W * 0.5f + 300, H * 0.5f - 28), new Vector2(110, 26), "装備・スキル", () => { audio?.UiPop(); SetTab(false); }, UiSkin.Blue, 12, false, 8);
                tabSupply = UiSkin.Button(card, "TabSupply", new Vector2(-W * 0.5f + 416, H * 0.5f - 28), new Vector2(110, 26), "補給", () => { audio?.UiPop(); SetTab(true); }, UiSkin.Btn, 12, false, 8);
            }
            ...
            if (hasSupply)
            {
                ...
                var cName = UiFactory.Label(coinCard, "Name", new Vector2(0, 52), new Vector2(300, 26), "路銀", 20, TextAnchor.MiddleCenter, UiSkin.Text);
                ...
                buyCredit = UiSkin.Button(coinCard, "Buy", new Vector2(0, -52), new Vector2(260, 38), "", () =>
                {
                    ...
                    audio?.UiPop();
                    SaveData.Save(m, audio);
                    note.text = $"路銀 {res.creditAmount} 枚を受け取りました";
                    note.color = UiSkin.Green;
                    RefreshAll();
                }, UiSkin.Gold, 15, false, 10);

                UiFactory.Label(supplyRoot, "SupNote", new Vector2(0, -H * 0.5f + 62), new Vector2(W - 60, 34),
                    $"{res.name} が尽きると街へ引き返す（進行はそのまま）。クレジットが尽きると力尽きて、章の最初からやり直しになる。", 12, TextAnchor.MiddleCenter, UiSkin.TextDim);
                ...
                rows.Add(() =>
                {
                    ...
                    cHave.text = $"{m.Credit:N0} 枚";
                    cDesc.text = $"1 口で {res.creditAmount} 枚";
                    UiSkin.SetButtonText(buyCredit, $"1 口 買う   {res.creditCost:N0}");
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                UiSkin.Chip(body, "Kind", new Vector2(-rowW * 0.5f + 44, rowH * 0.5f - 16), new Vector2(56, 18), isGear ? "装備" : "スキル", accent, UiSkin.Bg, 10);

                var nameT = UiFactory.Label(body, "Name", new Vector2(28, rowH * 0.5f - 16), new Vector2(210, 20), item.name, 15, TextAnchor.MiddleLeft, UiSkin.Text);
                ...
                var descT = UiFactory.Label(body, "Desc", new Vector2(16, -2), new Vector2(rowW - 40, 18), "", 12, TextAnchor.MiddleLeft, UiSkin.TextSub);
