...
- done: t01〜t20, c01〜c13, c16
- candidate（by: claude、未チェック）: c14（第 3 章のステージ名）/ c15（物語の上下の台詞が出ない段。案 1〜3）/ c17（AT が全 G の 55〜61%。目標の機械割が要る）/ c18（StageMapView.cs を消す。削除の OK が要る）/ c19（D の窓にランクの音）/ c20（別 PC の modules 定義との置き換え。push 待ち）/ c21（ハーネスに冒険なし・設定別）/ c22（PROJECT_STATE.md を今の状態に）
- 棚のページ: 済みは日付ごとに畳む（いちばん新しい日だけ開く）。「待ち」は別の段。候補には「Claude 案」の印
