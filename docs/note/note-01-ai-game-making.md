...
もうひとつは、壊れた箇所の全部。
AIが作ったものが、実際にどう壊れたか。14件を原因の型ごとに分類して並べます。
...
AIは、自分の変更を原因の第一候補にします。

これは効率がいいときもありますが、真犯人が別の層にいると、
いつまでも同じ場所を掘り続けます。

対処:「まず数値で層を特定してから直す」と指示に書く。
私は「見た目の不具合は、まず色や座標を測って、どの層の問題かを先に決める」と書いています。
...
| 件 | 状態 |
|---|---|
| 出玉のバランス | 設定1でも100%を大きく超えたまま。遊べるが、成立していない |
