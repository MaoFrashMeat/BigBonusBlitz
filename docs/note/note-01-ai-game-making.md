...
![現在のタイトル画面（Unity・2026年9月11日）。立ち絵は呼吸して、まばたきをして、髪が揺れる](images/21-title-unity.png)
*現在のタイトル画面（Unity・2026年9月11日）。立ち絵は呼吸して、まばたきをして、髪が揺れる*

- パチスロを土台にしたRPG。リールを回して敵と戦い、章立てのマップを進む
- Unity 6.3 / スクリプト80ファイル / 18,283行（9月14日時点）
- 敵との戦闘、装備のドロップ、呪いと祝福、AT（ボーナス後の連チャン）、街のショップ、物語
- 主人公の絵は9ポーズ。AIに描かせた
- 効果音は、素材を買わずに合成した
- コミットする前に、毎回100万ゲーム回して壊れないことを確認している

![冒険の画面。上でキャラが歩き、下でリールを回す。ここが本体](images/11-unity-game.png)
*冒険の画面。上でキャラが歩き、下でリールを回す。ここが本体*
...
もうひとつは、壊れた箇所の全部。
AIが作ったものが、実際にどう壊れたか。28件を原因の型ごとに分類して並べます。
...
* 絵は描けません
* 平日は本業があります。触れるのは、仕事の合間の1日2時間ほど。土日は2日で10時間ほど
* ゲーム制作は未経験。Unity は今回が初めて

時間の話も書いておきます。

作り始めたのが5月29日。この記事を書き始めたのが9月10日で、105日です。
そのうち約2ヶ月は、手が止まっていました（理由は次の章に書きます）。

実際に触っていたのは、50日ほど。
平日は1日あたり2時間くらい。土日は、2日合わせて10時間くらいです。

土日の10時間は、ずっと画面の前にいます。
AIが作業するのを見ながら、指示を出し続ける。私の作業はそれです。

この記事を書き始めた9月10日からの4日間で、コミットは149本増えました。
うち土日の2日で50本。10時間で50本なので、12分に1本です。
私が書いたわけではないので、この速さになります。
...
もうひとつ。AIは Unity を裏で動かして、画面を画像に描き出して自分で確認しています。
この記事に載せた画面の何枚かは、そのときの画像です。私が撮ったものではありません。
確認のために作った画像を捨てずに残しておくと、あとで記録になります。
...
![配列を並べる画面。右の検証結果に「全順:NG」が並んでいた頃（9月8日）](images/26-reel-editor.png)
*配列を並べる画面。右の検証結果に「全順:NG」が並んでいた頃（9月8日）*
...
![左が通常、右が目のパーツを消した瞬間。AIが確認用に Unity から描き出した画像](images/30-title-blink.png)
*左が通常、右が目のパーツを消した瞬間。AIが確認用に Unity から描き出した画像*
...
実機でどう見えるかは、この記事の冒頭に置いた画面のとおりです。
背景の城と空も3層に分けてあって、それぞれ違う速さで流れます。
...
できないものは、できる方に回す。それだけで先に進めます。

どのAIが何をできるかは、数ヶ月で入れ替わります。
1章のメモに書いたとおり、更新のたびにできることが増える。
だから「このAIはこれができない」も、日付つきで覚えておくようにしています。
...
![設定資料から切り出した、基準の1枚](images/27-character-reference.png)
*設定資料から切り出した、基準の1枚*
...
![冒険画面。端末の幅を決めたら、枠を全部置き直した](images/22-hud-before-after.png)
*冒険画面。端末の幅を決めたら、枠を全部置き直した*
...
![地図画面。3日で作り直した](images/23-map-before-after.png)
*地図画面。3日で作り直した*
...
![タイトル画面。同じ日の2時間](images/25-title-before-after.png)
*タイトル画面。同じ日の2時間*
...
そのときの依頼を、そのまま置いておきます。

> **そのときのメモ（2026-09-14）**
> 現状を把握し、背景の続きを作成してください。
> 背景はシームレスでスクロールしてもつなぎ目がないような構成で。
> Unity で朝、昼、夕方、夜、天候も左右されても大丈夫なような構成。
> 雨とか曇りとか色々ステージの状態によって見栄えを変えたい。
> また今の背景は少しリアル寄りなのでもう少しアニメチックにしたい。
> まずはテイストを決めたい。
> これらはほかのセクションでも行えるよう常に md にまとめて作業すること。

最後の1行が、この記事で書いてきた仕組みそのものです。
決めたことを、別のセッションでも使えるようにファイルに残す。もう癖になっています。

AIから出てきたのは3案でした。
セル画調、手描きアニメ調、絵本調。どれも違いました。

![AIが最初に出した3案。全部、不採用](images/28-style-board.png)
*AIが最初に出した3案。全部、不採用*

> 少しテイストが違うので参考画像をお渡しします。


紫の遺跡、青い洞窟、緑の森。「こんな感じです」と3枚渡しました。
...
私の返事はこれだけです。この方向で、残りの地点を作り直しています。

![参考を渡したあとの見本。3案とは別物になった](images/29-forest-v2.png)
*参考を渡したあとの見本。3案とは別物になった*

この見本は、奥・中・手前の3層に分けて描いてあります。
別々の速さで流すと、奥行きが出ます。

![3層に分けた状態。薄い灰色は透過](images/32-bg-layers.png)
*3層に分けた状態。薄い灰色は透過*
...
| 件 | 状態 |
|---|---|
| 出玉のバランス | 9月10日の実測（設定1・200万ゲーム・指示に従う）で165.3%。9日は132.6%だったので、あいだの調整で30ポイント上がった。遊べるが、成立していない |
| 設定差 | 設定5が142.2%で、設定1より低い。高い設定のほうが出ない逆転が起きている。作り直しが要る |
| 高設定のボーナス | 設定5・6の REG が終わらない（後述の㉖）。数値の調整は最後にまとめる方針なので、保留のまま |
| 背景のテイスト | 9月14日に見直しを始めた。AIの最初の3案は不採用。私が参考画像を3枚渡し、作り直した見本の方向で進めることにした。30地点のうち直ったのは1地点 |
...
| 数字 | 出どころ |
|---|---|
| 制作開始 2026-05-29 / 記事の開始 2026-09-10 / 最終更新 2026-09-14 | Gitのコミット履歴 |
| ブラウザ版 26コミット / 約2ヶ月の空白 | 同上 |
| 12,308行 / 47ファイル（9/10）→ 18,283行 / 80ファイル（9/14） | Unityプロジェクトのスクリプト実測 |
