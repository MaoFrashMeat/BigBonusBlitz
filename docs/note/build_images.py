...
def failure_types():
    im, dr = canvas(700)
    y = title(dr, "14件を並べて見えた、4つの型")
    y += 16

    rows = [("型A", "動くものは作るが、壊れないものは作らない",
             "画面上は正常に動く。壊れているのは平均値・境界・例外", "6件", "測る（通しテストを回す）"),
            ("型B", "直前に触った場所から疑い始める",
             "真犯人が別の層にいると、いつまでも同じ場所を掘る", "1件", "先に数値で層を特定させる"),
            ("型C", "文脈の外の制約を知らない",
    ...
    ly = y + 4 * (rh + gap) + 10
    dr.text((60, ly), "型A〜C は直せば済む。型D だけは、公開したあとだと直せない",
