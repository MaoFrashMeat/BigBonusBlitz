...
狙っているのは **iPhone の横持ち 2556 x 1179（19.5:9）**。

### Unity の Game ビュー

Game ビューの一覧は `Library/` に置かれ、git に載らない。
何もしないと PC ごとに違う比率で見ることになり、
「こちらでは収まっているのに、あちらでは切れる」が起きる。

Unity を開いたら 1 回だけ、メニューから実行する。

```
BBB > 画面サイズ > iPhone 横持ちに固定 (2556x1179)
```

一覧に無ければ足したうえで選ぶ。Game ビューが閉じていると足すだけになるので、
開いてからもう一度実行する。
...
| 設定 | 値 | なぜ |
|---|---|---|
| 既定の大きさ | 1278 x 590 | iPhone と同じ比のまま半分。どの画面にも収まる |
| 既定の大きさを使う | ON | OFF だと画面いっぱいで起動してしまう |
| 窓の大きさを変えられる | OFF | 比が崩れると配置が壊れる |
| 起動のしかた | 窓 | 全画面だと比が端末任せになる |

**iOS / Android ではこの値は使われない**（端末の解像度がそのまま入る）。
効くのは Windows / Mac のビルドと、エディタでの確認だけ。

### 舞台は 960 x 540 のまま

中身は `SafeStage` が 960 x 540 の枠に組み、端末に合わせて縮小している。
19.5:9 では左右に 105px ずつ余るが、そこは背景だけを見せる作りになっている
（`Assets/Scripts/BBB.Runtime/SafeStage.cs`）。
舞台の比を端末に合わせて広げるなら、配置の定数をすべて見直すことになる。
