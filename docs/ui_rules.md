...
2026-09-23追記: 見た目の完成基準は [AZURE_UI_QUALITY_STANDARD.md](AZURE_UI_QUALITY_STANDARD.md)。本人の「毎回言わなくても参考画像の質へ」という指定を今後のUI改修へ適用する。以下の事故防止と、素材・書体・余白・状態表現の品質を両方確認する。
...
## 15. ビューアの候補を減らす・並べ替えるときは、ブラウザに残った番号を守る

2026-09-14。図柄ビューアのブランク候補を 10 → 5 に減らしたら、本人のブラウザに残っていた選択（localStorage の
`remix: 5`）が範囲外になり、`s.cands[c].replace` で落ちて画面が真っ白になった。本人「図柄ビューアみれなくなったが？」。

### やること

- 保存した番号は読んだ直後に候補の数と突き合わせ、範囲外なら既定に戻す（`symbol_viewer.html` の `choice` の検査）
- 候補を指す値は番号よりファイル名で持つほうが安全。番号のままなら、候補を減らす変更のたびにこの検査を思い出す
- ビューアを直したら、古い localStorage を仕込んだ状態でも開けるか headless Chrome で確かめる（`--user-data-dir` を分けて seed する）

---

## 16. ビューアの操作パネルは 1 行 1 項目。横に詰め込まない

2026-09-14〜15。図柄ビューアの右の板に、点滅・層・縁取り・ランプの項目を flex-wrap で横に詰めていったら
「詰め込みすぎてて見づらい、分かりづらい」と言われた。項目が 10 を超えたら flex-wrap は破綻する。

### やること

- 1 行 1 項目。左にラベル列（幅を揃える）、右に操作。`grid-template-columns: 76px 1fr`
- 関連する項目は見出しつきの区画にまとめ、`<details>` で折りたためるようにする（よく使う区画だけ open）
- 太字の見出しを行の途中に置かない。「役の色」のような従属の選択は同じ行の右側に

---

## 17. 生成する .meta の型は「中身のあるファイル」から取る

2026-09-21。`tools/se_build.py` が新しい音の .meta を「同じ拡張子の既存 .meta の先頭」から写していたが、その先頭が
空（改行だけ 2 バイト）だったため、以後に作った 32 個の .meta が全部空になった。Unity が読み込めず、EE の攻撃・防御など
本人が選んだ音が鳴らなかった（本人「EE 時の攻撃やら防御やら音が聞こえない」）。

### やること

- 型にするファイルは `AudioImporter:`（画像なら `TextureImporter:`）を含むものだけ選ぶ。空・壊れたものは候補から外す
- 生成した直後に `find ... -name '*.meta' -size -10c` で空の .meta が無いことを確かめる
- `install_zone_art.py` / `text_build.py` も同じ型（既存の .meta を写す）なので、同じ検査を入れる
...
- [ ] ビューアの候補を減らしたとき、保存済みの番号が範囲外でも開けるか
- [ ] 操作パネルが 1 行 1 項目で、区画に見出しがあるか（横に詰め込んでいないか）
- [ ] 生成した .meta が空でないか（`-size -10c` で 0 件）

---

## 17. ビューアは描き直しでスクロールと折りたたみを戻さない

効果音ビューアで候補を 1 つクリックするたびに一覧が先頭へ戻り、本人に「この挙動何度もみる。どのセクションでも起こらないようにしろ」と言われた（2026-09-21）。
原因は innerHTML で一覧を作り直していること。**描き直す関数の頭で scrollTop と開いている details の見出しを覚え、描き直した後に戻す。**
型は `tools/se_viewer.html` の `render()`。fx / zone / symbol のビューアも同じ直しを入れた。新しいビューアを作るときも最初から入れる。
