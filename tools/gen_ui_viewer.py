...
document.addEventListener('mousemove', ev => {
  ...
  if (dragging.kind === 'move') {
    const nx = Math.round((dragging.ox + dx) / s) * s, ny = Math.round((dragging.oy + dy) / s) * s;
    moveTree(dragging.id, nx - r.x, ny - r.y);
  } else {
    // 右下の角を引く: 左上を固定して幅と高さを変える
    const nw = Math.max(20, Math.round((dragging.ow + dx) / s) * s), nh = Math.max(12, Math.round((dragging.oh - dy) / s) * s);
