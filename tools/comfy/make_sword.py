...
def draw(d, s):
    ...
    blade = P((cx, tip_y),
              ...
              (cx - half, guard_y),
              (cx - half, tip_y + 120))
    d.polygon(blade, fill=STEEL_LIGHT, outline=LINE, width=5 * s)
    # 刃の右半分を暗くして立体に見せる
    d.polygon(P((cx, tip_y), (cx + half, tip_y + 120), (cx + half, guard_y), (cx, guard_y)),
              fill=STEEL_DARK)
    d.polygon(blade, outline=LINE, width=5 * s)
    # 樋（中央の溝）
    d.line(P((cx, tip_y + 150), (cx, guard_y - 30)), fill=STEEL_LIGHT, width=9 * s)

    # 鍔。中央が厚く、両端が跳ね上がった形
    gy0, gy1 = guard_y, guard_y + 46
    d.polygon(P((cx - 96, gy0 + 26), (cx - 60, gy0), (cx + 60, gy0), (cx + 96, gy0 + 26),
               (cx + 60, gy1), (cx - 60, gy1)),
              fill=GOLD, outline=LINE, width=5 * s)
    d.polygon(P((cx - 96, gy0 + 26), (cx - 60, gy1), (cx + 60, gy1), (cx + 96, gy0 + 26)),
              fill=GOLD_DARK)
    d.polygon(P((cx - 96, gy0 + 26), (cx - 60, gy0), (cx + 60, gy0), (cx + 96, gy0 + 26),
               (cx + 60, gy1), (cx - 60, gy1)),
              outline=LINE, width=5 * s)
    # 鍔の中央の宝玉
    d.ellipse(P((cx - 17, gy0 + 6), (cx + 17, gy0 + 40)), fill=CYAN, outline=LINE, width=4 * s)

    # 柄
    hy0, hy1 = gy1, gy1 + 186
    d.rounded_rectangle(P((cx - 22, hy0), (cx + 22, hy1)), radius=10 * s,
                        fill=NAVY, outline=LINE, width=5 * s)
    for k in range(4):                          # 柄の巻き
        y = hy0 + 26 + k * 40
        d.line(P((cx - 22, y), (cx + 22, y - 14)), fill=GOLD_DARK, width=5 * s)

    # 柄頭
    d.ellipse(P((cx - 32, hy1 - 8), (cx + 32, hy1 + 56)), fill=GOLD, outline=LINE, width=5 * s)
    d.ellipse(P((cx - 13, hy1 + 11), (cx + 13, hy1 + 37)), fill=CYAN, outline=LINE, width=4 * s)
