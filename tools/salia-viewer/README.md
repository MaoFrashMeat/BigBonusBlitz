...
目元と顔の中心は、全ての揺れを計算した後に共通の保護範囲で固定します。前髪だけでなく、後ろ髪・呼吸・服の変形も目には伝わりません。目パチはテクスチャの補間として維持します。UnityとWebは同じ保護範囲を使い、Unityの描画検証では最大設定の10時刻で開眼・閉眼それぞれの目元が静止基準と一致することを確認します。
